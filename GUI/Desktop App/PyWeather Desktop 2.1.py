...
import requests
import folium
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QComboBox, QMenuBar, QAction
...
class WeatherMapApp(QWidget):
    ...
    def init_ui(self):
        self.setWindowTitle('PyWeather 2.1')
        ...
        # Add action for temperature unit toggle
        temperature_unit_action = QAction('Toggle Temperature Unit', self)
        temperature_unit_action.triggered.connect(self.toggle_temperature_unit)
        options_menu.addAction(temperature_unit_action)
    ...
    def display_weather(self, weather_data):
        ...
        temperature = f"Temperature: {weather_data['main']['temp']}°{self.get_temperature_unit()}"
    ...
    def toggle_temperature_unit(self):
        if self.units == "metric":
            self.units = "imperial"  # Switch to Fahrenheit
        else:
            self.units = "metric"  # Switch to Celsius

        self.get_weather_and_map()
